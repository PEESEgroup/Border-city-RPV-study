...
def plot_panel_c(city_results: dict[str, dict[str, np.ndarray]], out_path: Path) -> None:
    ...
    for i, city in enumerate(ordered_cities):
        ...
        ax_c_right.barh(
            ...
            error_kw={"elinewidth": 1.0, "ecolor": color, "capsize": 0, "alpha": 0.95},
        )
        ax_c_right.barh(
            y=y1,
            width=export_mid,
    ...
    ax_c_left.set_yticklabels([CITY_DISPLAY_NAMES.get(city, city) for city in ordered_cities], fontsize=10)
    ...
    ax_c_left.set_xlim(0.0, left_max * 1.06)
